...
public protocol WeekViewFlowLayoutDelegate: AnyObject {
    ...
    func collectionView(
        ...
        layout: AWWeekFlowLayout,
        dayForSection section: Int
    ) -> Date
    /// Get the start time for given item indexPath
    func collectionView(
        _ collectionView: UICollectionView,
        layout: AWWeekFlowLayout,
        startTimeForItemAtIndexPath indexPath: IndexPath
    ) -> Date
    /// Get the end time for given item indexPath
    func collectionView(
        _ collectionView: UICollectionView,
        layout: AWWeekFlowLayout,
        endTimeForItemAtIndexPath indexPath: IndexPath
    ) -> Date
    /// TODO: Get the cell type for given item indexPath (Used for different cell types in the future)
    func collectionView(
        _ collectionView: UICollectionView,
        layout: AWWeekFlowLayout,
        cellTypeForItemAtIndexPath indexPath: IndexPath
    ) -> String
}

open class AWWeekFlowLayout: UICollectionViewFlowLayout {
    ...
    public var hourGridDivision: AWHourGridDivision!
    ...
    open var defaultHourGridDivision: AWHourGridDivision { return .noneDiv }
    ...
    open var contentsMargin: UIEdgeInsets {
    ...
    open var maxSectionHeight: CGFloat {
        ...
        return columnHeaderHeight + height + contentsMargin.top + contentsMargin.bottom
            + allDayHeaderHeight
    }
    ...
    public init(
        hourHeight: CGFloat? = nil,
        rowHeaderWidth: CGFloat? = nil,
        columnHeaderHeight: CGFloat? = nil,
        hourGridDivision: AWHourGridDivision? = nil
    ) {
        super.init()
    ...
    private func initializeMinuteTick() {
        if #available(iOS 10.0, *) {
            minuteTimer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) {
                [weak self] _ in
                self?.minuteTick()
            }
        } else {
            minuteTimer = WeakTimer.scheduledTimer(timeInterval: 60, target: self, repeats: true) {
                [weak self] _ in
                self?.minuteTick()
            }
        }
    ...
    override open func prepare() {
        ...
        if needsToPopulateAttributesForAllSections {
            prepareHorizontalTileSectionLayoutForSections(
                NSIndexSet(
                    indexesIn:
    ...
    open func prepareHorizontalTileSectionLayoutForSections(_ sectionIndexes: NSIndexSet) {
        ...
        sectionIndexes.enumerate(_:) { (section, _) in
            ...
            let timeY =
                calendarContentMinY
                + (CGFloat(currentTimeComponents.hour!).toDecimal1Value() * hourHeight
                    + CGFloat(currentTimeComponents.minute!) * minuteHeight)
            let currentTimeHorizontalGridlineMinY =
                timeY - (defaultGridThickness / 2.0).toDecimal1Value()
                - defaultCurrentTimeLineHeight / 2
            (attributes, currentTimeLineAttributes) = layoutAttributesForSupplemantaryView(
                at: IndexPath(item: 0, section: section),
                ofKind: AWSupplementaryViewKind.currentTimeline,
                withItemCache: currentTimeLineAttributes
            )
            attributes.frame = CGRect(
                x: sectionMinX,
                y: currentTimeHorizontalGridlineMinY,
                width: sectionWidth,
                height: defaultCurrentTimeLineHeight
            )
            attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.currentTimeline)
        }

        // Corner Header
        (attributes, cornerHeaderAttributes) = layoutAttributesForSupplemantaryView(
            at: IndexPath(item: 0, section: 0),
            ofKind: AWSupplementaryViewKind.cornerHeader,
            withItemCache: cornerHeaderAttributes
        )
        attributes.frame = CGRect(
            origin: collectionView.contentOffset,
            size: CGSize(width: rowHeaderWidth, height: columnHeaderHeight)
        )
        attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.cornerHeader)

        // Row header
        let rowHeaderMinX = fmax(collectionView.contentOffset.x, 0)

        for rowHeaderIndex in 0...24 {
            (attributes, rowHeaderAttributes) = layoutAttributesForSupplemantaryView(
                at: IndexPath(item: rowHeaderIndex, section: 0),
                ofKind: AWSupplementaryViewKind.rowHeader,
                withItemCache: rowHeaderAttributes
            )
            let rowHeaderMinY =
                calendarContentMinY + hourHeight * CGFloat(rowHeaderIndex)
                - (hourHeight / 2.0).toDecimal1Value()
            attributes.frame = CGRect(
                x: rowHeaderMinX,
                y: rowHeaderMinY,
            ...
            attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.rowHeader)
        }

        // Row Header Background
        (attributes, rowHeaderBackgroundAttributes) = layoutAttributesForDecorationView(
            at: IndexPath(item: 0, section: 0),
            ofKind: AWDecorationViewKind.rowHeaderBackground,
        ...
        attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.rowHeaderBackground)

        // All-Day header
        let allDayHeaderMinY = fmax(
            collectionView.contentOffset.y + columnHeaderHeight,
            columnHeaderHeight
        )
        ...
        sectionIndexes.enumerate(_:) { (section, _) in
            ...
            (attributes, allDayHeaderAttributes) =
                layoutAttributesForSupplemantaryView(
                    at: IndexPath(item: 0, section: section),
                    ofKind: AWSupplementaryViewKind.allDayHeader,
            ...
            attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.allDayHeader)
        }
        ...
        (attributes, allDayHeaderBackgroundAttributes) =
            layoutAttributesForDecorationView(
                at: IndexPath(item: 0, section: 0),
                ofKind: AWDecorationViewKind.allDayHeaderBackground,
        ...
        attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.allDayHeaderBackground)

        (attributes, allDayCornerAttributes) =
            layoutAttributesForDecorationView(
                at: IndexPath(item: 0, section: 0),
                ofKind: AWDecorationViewKind.allDayCorner,
        ...
        attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.allDayCorner)

        // column header background
        (attributes, columnHeaderBackgroundAttributes) = layoutAttributesForDecorationView(
            at: IndexPath(item: 0, section: 0),
            ofKind: AWDecorationViewKind.columnHeaderBackground,
            withItemCache: columnHeaderBackgroundAttributes
        )
        let attributesHeight =
            columnHeaderHeight
            + (collectionView.contentOffset.y < 0 ? abs(collectionView.contentOffset.y) : 0)
        attributes.frame = CGRect(
            origin: collectionView.contentOffset,
            size: CGSize(width: collectionView.frame.width, height: attributesHeight)
        )
        attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.columnHeaderBackground)
        ...
        sectionIndexes.enumerate(_:) { (section, _) in
            let sectionMinX = calendarContentMinX + sectionWidth * CGFloat(section)
            (attributes, columnHeaderAttributes) = layoutAttributesForSupplemantaryView(
                at: IndexPath(item: 0, section: section),
                ofKind: AWSupplementaryViewKind.columnHeader,
            ...
            attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.columnHeader)

            layoutVerticalGridLinesAttributes(
                section: section,
                sectionX: sectionMinX,
                calendarGridMinY: calendarGridMinY,
                sectionHeight: sectionHeight
            )
            layoutItemsAttributes(
                section: section,
                sectionX: sectionMinX,
                calendarStartY: calendarGridMinY
            )
        }

        layoutHorizontalGridLinesAttributes(
            calendarStartX: calendarContentMinX,
            calendarStartY: calendarContentMinY
        )
    }
    ...
    func layoutItemsAttributes(section: Int, sectionX: CGFloat, calendarStartY: CGFloat) {
        ...
        for item in 0..<collectionView!.numberOfItems(inSection: section) {
            ...
            (attributes, itemAttributes) = layoutAttributesForCell(
                at: itemIndexPath,
                withItemCache: itemAttributes
            )
            ...
            if itemEndTime.day! != itemStartTime.day! {
                endHourY =
                    CGFloat(Calendar.current.maximumRange(of: .hour)!.count) * hourHeight + CGFloat(
                        itemEndTime.hour!
                    ) * hourHeight
            } else {
            ...
            let itemMinY = (startHourY + startMinuteY + calendarStartY + itemMargin.top)
                .toDecimal1Value()
            let itemMaxX = (itemMinX + (sectionWidth - (itemMargin.left + itemMargin.right)))
                .toDecimal1Value()
            let itemMaxY = (endHourY + endMinuteY + calendarStartY - itemMargin.bottom)
                .toDecimal1Value()
            ...
            attributes.zIndex = zIndexForElementKind(AWSupplementaryViewKind.eventCell)
        ...
        adjustItemsForOverlap(
            ...
            currentSectionZ: zIndexForElementKind(AWSupplementaryViewKind.eventCell)
        )
    }

    func layoutVerticalGridLinesAttributes(
        section: Int,
        sectionX: CGFloat,
        ...
        (attributes, verticalGridlineAttributes) = layoutAttributesForDecorationView(
            at: IndexPath(item: 0, section: section),
            ofKind: AWDecorationViewKind.verticalGridline,
            withItemCache: verticalGridlineAttributes
        )
        attributes.frame = CGRect(
            x: (sectionX - defaultGridThickness / 2.0).toDecimal1Value(),
            y: calendarGridMinY,
            width: defaultGridThickness,
            height: sectionHeight
        )
        attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.verticalGridline)
    }

    func layoutHorizontalGridLinesAttributes(calendarStartX: CGFloat, calendarStartY: CGFloat) {
        var horizontalGridlineIndex = 0
        let calendarGridWidth =
            collectionViewContentSize.width - rowHeaderWidth - contentsMargin.left
            - contentsMargin.right
        var attributes = UICollectionViewLayoutAttributes()

        for hour in 0...24 {
            (attributes, horizontalGridlineAttributes) = layoutAttributesForDecorationView(
                at: IndexPath(item: horizontalGridlineIndex, section: 0),
                ofKind: AWDecorationViewKind.horizontalGridline,
                withItemCache: horizontalGridlineAttributes
            )
            let horizontalGridlineXOffset = calendarStartX
            let horizontalGridlineMinX = fmax(
                horizontalGridlineXOffset,
                collectionView!.contentOffset.x + horizontalGridlineXOffset
            )
            let horizontalGridlineMinY =
                (calendarStartY + (hourHeight * CGFloat(hour)))
                - (defaultGridThickness / 2.0).toDecimal1Value()
            let horizontalGridlineWidth = fmin(calendarGridWidth, collectionView!.frame.width)

            attributes.frame = CGRect(
                x: horizontalGridlineMinX,
                y: horizontalGridlineMinY,
                width: horizontalGridlineWidth,
                height: defaultGridThickness
            )
            attributes.zIndex = zIndexForElementKind(AWDecorationViewKind.horizontalGridline)
            horizontalGridlineIndex += 1

            if hourGridDivision.rawValue > 0 {
                horizontalGridlineIndex = drawHourDividersAtGridLineIndex(
    ...
    func drawHourDividersAtGridLineIndex(
        _ gridlineIndex: Int,
        hour: Int,
        startX calendarStartX: CGFloat,
        startY calendarStartY: CGFloat,
        ...
        for division in 1..<numberOfDivisions {
            ...
            (attributes, horizontalGridlineAttributes) = layoutAttributesForDecorationView(
                at: horizontalGridlineIndexPath,
                ofKind: AWDecorationViewKind.horizontalGridline,
                withItemCache: horizontalGridlineAttributes
            )
            let horizontalGridlineMinY =
                (calendarStartY + (divisionHeight * CGFloat(division))
                - (defaultGridThickness / 2.0))
                .toDecimal1Value()
            attributes.frame = CGRect(
                x: calendarStartX,
                y: horizontalGridlineMinY,
    ...
    override open var collectionViewContentSize: CGSize {
        return CGSize(
            width: rowHeaderWidth + sectionWidth * CGFloat(collectionView!.numberOfSections),
    ...
    override open func layoutAttributesForSupplementaryView(
        ...
        switch elementKind {
        case AWSupplementaryViewKind.columnHeader:
            return columnHeaderAttributes[indexPath]
        case AWSupplementaryViewKind.rowHeader:
            return rowHeaderAttributes[indexPath]
        case AWSupplementaryViewKind.cornerHeader:
            return cornerHeaderAttributes[indexPath]
        case AWSupplementaryViewKind.allDayHeader:
            return allDayHeaderAttributes[indexPath]
        case AWSupplementaryViewKind.currentTimeline:
    ...
    override open func layoutAttributesForDecorationView(
        ...
        switch elementKind {
        case AWDecorationViewKind.verticalGridline:
            return verticalGridlineAttributes[indexPath]
        case AWDecorationViewKind.horizontalGridline:
            return horizontalGridlineAttributes[indexPath]
        case AWDecorationViewKind.rowHeaderBackground:
            return rowHeaderBackgroundAttributes[indexPath]
        case AWDecorationViewKind.columnHeaderBackground:
            return columnHeaderBackgroundAttributes[indexPath]
        case AWDecorationViewKind.allDayHeaderBackground:
            return allDayHeaderBackgroundAttributes[indexPath]
        case AWDecorationViewKind.allDayCorner:
    ...
    // MARK: - Layout
    func layoutAttributesForCell(at indexPath: IndexPath, withItemCache itemCache: AttDic) -> (
        UICollectionViewLayoutAttributes, AttDic
    ) {
    ...
    func layoutAttributesForDecorationView(
        customAttributes: UICollectionViewLayoutAttributes,
        withItemCache itemCache: AttDic
    ) -> (UICollectionViewLayoutAttributes, AttDic) {
        var _itemCache = itemCache
    ...
    private func layoutAttributesForSupplemantaryView(
        at indexPath: IndexPath,
        ofKind kind: String,
        withItemCache itemCache: AttDic
    ) -> (UICollectionViewLayoutAttributes, AttDic) {
    ...
    */
    open func adjustItemsForOverlap(
        _ sectionItemAttributes: [UICollectionViewLayoutAttributes],
        inSection: Int,
        sectionMinX: CGFloat,
        currentSectionZ: Int
    ) {
        let (maxOverlapIntervalCount, overlapGroups) = groupOverlapItems(
            items: sectionItemAttributes
        )
        ...
        for index in 1..<sortedOverlapGroups.count {
            ...
            let availableRanges = getAvailableRanges(
                sectionRange: sectionMinX...sectionMinX + sectionWidth,
                adjustedRanges: adjustedRanges
            )
            let minItemDivisionWidth = (sectionWidth / CGFloat(largestOverlapCountGroup.count))
                .toDecimal1Value()
            var i = 0
            var j = 0
            while i < unadjustedItems.count && j < availableRanges.count {
                ...
                if leftUnadjustedItemsCount <= availableMaxItemsCount {
                    ...
                    setItemsAdjustedAttributes(
                        ...
                        items: Array(unadjustedItems[i..<unadjustedItems.count]),
                        currentMinX: availableRange.lowerBound,
                        sectionZ: &sectionZ,
                        adjustedItems: &adjustedItems
                    )
                    break
                    ...
                    setItemsAdjustedAttributes(
                        fullWidth: availableWidth,
                        items: Array(unadjustedItems[i..<i + availableMaxItemsCount]),
                        currentMinX: availableRange.lowerBound,
                        sectionZ: &sectionZ,
                        adjustedItems: &adjustedItems
                    )
    ...
    func getAvailableRanges(
        sectionRange: ClosedRange<CGFloat>,
        adjustedRanges: [ClosedRange<CGFloat>]
    ) -> [ClosedRange<CGFloat>] {
        ...
        for adjustedRange in sortedAdjustedRange {
            ...
            if adjustedRange.lowerBound > lastAvailableRange.lowerBound + itemMargin.left
                + itemMargin.right
            {
                ...
                if adjustedRange.upperBound + itemMargin.right >= lastAvailableRange.upperBound {
                    ...
                    let leftAvailableRange =
                        lastAvailableRange.lowerBound...adjustedRange.lowerBound
                    currentAvailableRanges.append(leftAvailableRange)
                } else {
                    // Adjusted range is in middle of the last available range
                    let leftAvailableRange =
                        lastAvailableRange.lowerBound...adjustedRange.lowerBound
                    let rightAvailableRange =
                        adjustedRange.upperBound...lastAvailableRange.upperBound
    ...
    private func setItemsAdjustedAttributes(
        fullWidth: CGFloat,
        ...
        for (index, itemAttribute) in items.enumerated() {
            itemAttribute.frame.origin.x =
                (currentMinX + itemMargin.left + CGFloat(index) * divisionWidth).toDecimal1Value()
    ...
    private func maxOverlapIntervalCount(startY: [CGFloat], endY: [CGFloat]) -> Int {
        ...
        let sortedStartY = startY.sorted()
        let sortedEndY = endY.sorted()
    ...
    func groupOverlapItems(items: [UICollectionViewLayoutAttributes]) -> (
    ...
    open func rectForSection(_ section: Int) -> CGRect {
        return CGRect(
            x: rowHeaderWidth + sectionWidth * CGFloat(section),
            y: 0,
            width: sectionWidth,
    ...
    /// Internal use only, use getDateForSection in AWWeekView instead
    private func daysForSection(_ section: Int) -> DateComponents {
        ...
        let dayDateComponents = Calendar.current.dateComponents(
            [.year, .month, .day],
            from: startOfDay
        )
    ...
    private func startTimeForIndexPath(_ indexPath: IndexPath) -> DateComponents {
        if cachedStartTimeDateComponents[indexPath] != nil {
            ...
            if let date = delegate?
                .collectionView(
                    collectionView!,
                    layout: self,
                    startTimeForItemAtIndexPath: indexPath
                )
            {
                cachedStartTimeDateComponents[indexPath] = Calendar.current.dateComponents(
                    [.day, .hour, .minute],
                    from: date
                )
                return cachedStartTimeDateComponents[indexPath]!
    ...
    private func endTimeForIndexPath(_ indexPath: IndexPath) -> DateComponents {
        if cachedEndTimeDateComponents[indexPath] != nil {
            ...
            if let date = delegate?
                .collectionView(collectionView!, layout: self, endTimeForItemAtIndexPath: indexPath)
            {
                cachedEndTimeDateComponents[indexPath] = Calendar.current.dateComponents(
                    [.day, .hour, .minute],
                    from: date
                )
                return cachedEndTimeDateComponents[indexPath]!
    ...
    open func dateForColumnHeader(at indexPath: IndexPath) -> Date {
        let day = delegate?
            .collectionView(collectionView!, layout: self, dayForSection: indexPath.section)
    ...
    open func zIndexForElementKind(_ kind: String) -> Int {
        switch kind {
        case AWSupplementaryViewKind.cornerHeader, AWDecorationViewKind.allDayCorner:
