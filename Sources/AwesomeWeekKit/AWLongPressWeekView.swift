...
public protocol AWLongPressViewDelegate: AnyObject {
    ...
    /// - Parameters:
    ///   - weekView: current long pressed AWLongPressWeekView
    ///   - startDate: the startDate of the event when gesture ends
    func weekView(_ weekView: AWLongPressWeekView, didEndAddNewLongPressAt startDate: Date)
    ...
    ///   - startDate: the startDate of the event when gesture ends
    func weekView(
        _ weekView: AWLongPressWeekView,
        editingEvent: AWEvent,
    ...
    func weekView(
        _ weekView: AWLongPressWeekView,
        longPressType: AWLongPressWeekView.LongPressType,
        didCancelLongPressAt startDate: Date
    )
}

public protocol AWLongPressViewDataSource: AnyObject {
    ...
    func weekView(_ weekView: AWLongPressWeekView, viewForAddNewLongPressAt startDate: Date)
        -> UIView
...
extension AWLongPressViewDelegate {
    // Keep them optional
    ...
    public func weekView(_ weekView: AWLongPressWeekView, didEndAddNewLongPressAt startDate: Date) {
    }
    public func weekView(
        _ weekView: AWLongPressWeekView,
        editingEvent: AWEvent,
...
extension AWLongPressViewDataSource {
...
open class AWLongPressWeekView: AWWeekView {
    ...
    public weak var longPressDelegate: AWLongPressViewDelegate?
    public weak var longPressDataSource: AWLongPressViewDataSource?
    ...
    open var longPressTopMarginY: CGFloat {
        return flowLayout.columnHeaderHeight + flowLayout.allDayHeaderHeight
    }
    ...
    private func updateTimeLabelPosition(pointInSelfView: CGPoint) {
        let isOutsideLeftMargin =
            pointInSelfView.x - pressPosition!.xToViewLeft < longPressLeftMarginX
        longPressTimeLabel.textAlignment = isOutsideLeftMargin ? .right : .left

        let labelHeight = longPressTimeLabel.frame.height
        let isBeyondTopMargin =
            pointInSelfView.y - pressPosition!.yToViewTop - labelHeight < longPressTopMarginY
    ...
    */
        ...
        if direction == .up || direction == .down {
            ...
            if direction == .up {
                ...
                collectionView.setContentOffset(
                    CGPoint(x: currentOffset.x, y: yOffset),
                    animated: true
                )
            } else {
            ...
            let contentOffsetXWithScrollableEdges = min(
                max(contentOffsetX, scrollableEdges.leftX ?? -1),
                scrollableEdges.rightX ?? CGFloat.greatestFiniteMagnitude
            )
            if contentOffsetXWithScrollableEdges == currentOffset.x {
                ...
                collectionView.setContentOffset(
                    CGPoint(x: contentOffsetXWithScrollableEdges, y: currentOffset.y),
                    animated: true
                )
            }
        }
    ...
    open func initLongPressView(
        ...
        let longPressView =
            type == .move
            ? longPressDataSource!
                .weekView(self, movingCell: selectedCell!, viewForMoveLongPressAt: startDate)
        ...
        let textWidth =
            UILabel.getLabelWidth(labelHeight, font: longPressTimeLabel.font, text: "23:59") + 5
        let timeLabelWidth = max(selectedCell?.bounds.width ?? flowLayout.sectionWidth, textWidth)
        longPressTimeLabel.frame = CGRect(
            x: 0,
            y: -labelHeight,
    ...
    open func getDateForPoint(pointCollectionView: CGPoint, pointSelfView: CGPoint) -> Date {
        let yearMonthDay = getDateForPointX(
            xCollectionView: pointCollectionView.x,
            xSelfView: pointSelfView.x
        )
    ...
    // scrollViewDidEndScrollingAnimation won't be called in AWWeekView, then should load page here
    ...
    open func collectionView(
        ...
        if isOriginalMovingCell(cell) {
            ...
            if let index = currentEditingInfo.allOpacityContentViews.firstIndex(where: {
                $0 == cellContentView
            }) {
    ...
    private func isOriginalMovingCell(_ cell: UICollectionViewCell) -> Bool {
        if let cell = cell as? AWLongPressEventCell {
...
extension AWLongPressWeekView: UIGestureRecognizerDelegate {

    // Override this function to customise gesture begin conditions
    override open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer)
        -> Bool
    {
        ...
        if gestureRecognizer.state == .possible {
            ...
            let isOutsideBeginArea =
                pointInSelfView.x < longPressLeftMarginX
                || pointInSelfView.x > longPressRightMarginX
                || pointInSelfView.y < longPressTopMarginY
    ...
    @objc private func handleLongPressGesture(_ gestureRecognizer: UILongPressGestureRecognizer) {
        ...
        if state == .began {
            ...
            currentEditingInfo.cellSize =
                currentLongPressType == .move
                ? currentMovingCell.frame.size
                : CGSize(
                    width: flowLayout.sectionWidth,
                    height: flowLayout.hourHeight * CGFloat(addNewDurationMins) / 60
                )
            pressPosition =
                currentLongPressType == .move
                ? (
            ...
            longPressView = initLongPressView(
                selectedCell: currentMovingCell,
                type: currentLongPressType,
                startDate: longPressViewStartDate
            )
            ...
            longPressView.center = CGPoint(
                x: pointInSelfView.x - pressPosition!.xToViewLeft
                    + currentEditingInfo.cellSize.width / 2,
                y: pointInSelfView.y - pressPosition!.yToViewTop
                    + currentEditingInfo.cellSize.height / 2
            )
            if currentLongPressType == .move {
                currentEditingInfo.event = (currentMovingCell as! AWLongPressEventCell).event
                getCurrentMovingCells()
                    .forEach {
            ...
            if currentLongPressType == .addNew {
                ...
                longPressDelegate?
                    .weekView(
                        self,
                        editingEvent: currentEditingInfo.event,
    ...
    private func getLongPressViewStartDate(pointInCollectionView: CGPoint, pointInSelfView: CGPoint)
        -> Date
    {
        let longPressViewTopDate = getDateForPoint(
            pointCollectionView: CGPoint(
                x: pointInCollectionView.x,
                y: pointInCollectionView.y - pressPosition!.yToViewTop
            ),
