...
public protocol AWWeekViewDelegate: AnyObject {
...
open class AWWeekView: UIView {
    ...
    public var collectionView: AWCollectionView!
    public var flowLayout: AWWeekFlowLayout!
    ...
    */
    ...
    public var scrollableRange: (startDate: Date?, endDate: Date?) {
        didSet {
            self.scrollableRange = (
                self.scrollableRange.startDate?.startOfDay, self.scrollableRange.endDate?.startOfDay
            )
    ...
    public var currentTimelineType: AWCurrentTimelineType! {
        didSet {
            let viewClass =
                ...
                ? AWCurrentTimelineSection.self : AWCurrentTimelinePage.self
            self.collectionView.register(
                viewClass,
                forSupplementaryViewOfKind: AWSupplementaryViewKind.currentTimeline,
                withReuseIdentifier: AWSupplementaryViewKind.currentTimeline
            )
        }
    }
    ...
    public var notAllDayEventsBySection = [Date: [AWAllDayEvent]]()
    public var allDayEventsBySection = [Date: [AWAllDayEvent]]()

    public weak var baseDelegate: AWWeekViewDelegate?
    open var contentViewWidth: CGFloat {
        return frame.width - flowLayout.rowHeaderWidth - flowLayout.contentsMargin.left
            - flowLayout.contentsMargin.right
    }
    private var isFirstAppear: Bool = true
    internal var isAllDaySupported: Bool!
    internal var AWWeekViewScrollDirection: AWWeekViewScrollDirection?
    ...
    open func setup() {
        flowLayout = AWWeekFlowLayout()
        flowLayout.delegate = self

        collectionView = AWCollectionView(frame: bounds, collectionViewLayout: flowLayout)
    ...
    open func registerViewClasses() {
        ...
        self.collectionView.registerSupplimentaryViews([
            AWColumnHeader.self, AWCornerHeader.self, AWRowHeader.self, AWAllDayHeader.self,
        ])

        // decoration
        flowLayout.registerDecorationViews([
            AWColumnHeaderBackground.self, AWRowHeaderBackground.self,
            AWAllDayHeaderBackground.self, AWAllDayCorner.self,
        ])
        flowLayout.register(
            AWGridLine.self,
            forDecorationViewOfKind: AWDecorationViewKind.verticalGridline
        )
        flowLayout.register(
            AWGridLine.self,
            forDecorationViewOfKind: AWDecorationViewKind.horizontalGridline
        )
    }
    ...
    private func getSectionWidth() -> CGFloat {
        ...
        let rowHeaderWidth =
            frame.width - flowLayout.contentsMargin.left - flowLayout.contentsMargin.right
            - sectionWidth * CGFloat(numOfDays)
    ...
    /**
     ...
     - Parameters:
        ...
        - allEvents: The dictionary of all the events for present. AWWeekViewHelper.getIntraEventsByDate can help transform the data
    ...
    */
    open func setupCalendar(
        numOfDays: Int,
        setDate: Date,
        allEvents: [Date: [AWEvent]],
        scrollType: AWWeekViewScrollType = .page,
        firstDayOfWeek: AWDayOfWeek? = nil,
        currentTimelineType: AWCurrentTimelineType = .section,
        ...
        DispatchQueue.main.async { [unowned self] in
            ...
            AWWeekViewHelper.viewTransitionHandler(
    ...
    open func setupAllDayEvents() {
        ...
        for (date, events) in allEventsBySection {
            guard let allDayEvents = events as? [AWAllDayEvent] else { continue }
    ...
    open func updateAllDayBar(isScrolling: Bool) {
        ...
        getDatesInCurrentPage(isScrolling: isScrolling)
            .forEach {
        ...
        if newAllDayHeader != flowLayout.allDayHeaderHeight {
            ...
            if !isScrolling
                || !willEffectContentSize(
                    difference: flowLayout.allDayHeaderHeight - newAllDayHeader
                )
            {
    ...
    /// Update collectionViewLayout with custom flowLayout. For some other values like gridThickness and contentsMargin, please inherit from AWWeekFlowLayout to change the default value
    /// - Parameter flowLayout: Custom CalendarWeekView flowLayout
    open func updateFlowLayout(_ flowLayout: AWWeekFlowLayout) {
    ...
    open func forceReload(reloadEvents: [Date: [AWEvent]]? = nil) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            strongSelf.collectionView.setContentOffsetWithoutDelegate(
                CGPoint(x: strongSelf.contentViewWidth, y: strongSelf.getYOffset()),
                animated: false
            )
    ...
    private func willEffectContentSize(difference: CGFloat) -> Bool {
        return collectionView.contentOffset.y + difference + collectionView.bounds.height
    ...
    private func getYOffset() -> CGFloat {
        ...
        let bottomOffset =
            flowLayout.collectionViewContentSize.height - collectionView.bounds.height
    ...
    open func scrollWeekView(to time: Date) {
        ...
        let maxOffsetY =
            ...
            + flowLayout.columnHeaderHeight + flowLayout.allDayHeaderHeight
            + flowLayout.contentsMargin.bottom + flowLayout.contentsMargin.top
    ...
    open func getCurrentEvent(with indexPath: IndexPath) -> AWEvent? {
        let date = flowLayout.dateForColumnHeader(at: indexPath)
        return isAllDaySupported
            ? notAllDayEventsBySection[date]?[indexPath.row]
            : allEventsBySection[date]?[indexPath.row]
    }
    ...
    open func getDatesInCurrentPage(isScrolling: Bool) -> [Date] {
        ...
        let endDate = getDateForContentOffsetX(
            collectionView.contentOffset.x + contentViewWidth - 1
        )
        repeat {
    ...
    */
    ...
    open func updateFirstDayOfWeek(setDate: Date, firstDayOfWeek: AWDayOfWeek?) {
    ...
    open func getDateForPointY(_ yCollectionView: CGFloat) -> (hour: Int, minute: Int) {
        ...
        var adjustedY =
            yCollectionView - flowLayout.columnHeaderHeight - flowLayout.contentsMargin.top
            - flowLayout.allDayHeaderHeight
        let minY: CGFloat = 0
        // contentSize includes all reusableView, margin and scrollable area
        let maxY =
            collectionView.contentSize.height - flowLayout.contentsMargin.top
            - flowLayout.contentsMargin.bottom - flowLayout.allDayHeaderHeight
            - flowLayout.columnHeaderHeight
...
extension AWWeekView: UICollectionViewDataSource {
    ...
    open func collectionView(
        ...
        case AWSupplementaryViewKind.columnHeader:
            if let columnHeader = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
        ...
        case AWSupplementaryViewKind.rowHeader:
            if let rowHeader = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: kind,
                for: indexPath
            ) as? AWRowHeader {
        ...
        case AWSupplementaryViewKind.cornerHeader:
            if let cornerHeader = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: kind,
                for: indexPath
            ) as? AWCornerHeader {
        ...
        case AWSupplementaryViewKind.allDayHeader:
            if let alldayHeader = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: kind,
                for: indexPath
            ) as? AWAllDayHeader {
        ...
        case AWSupplementaryViewKind.currentTimeline:
            if currentTimelineType == .page {
                if let currentTimeline = collectionView.dequeueReusableSupplementaryView(
                    ofKind: kind,
                    withReuseIdentifier: kind,
                    for: indexPath
                ) as? AWCurrentTimelinePage {
                    view = getPageTypeCurrentTimeline(
                ...
                if let currentTimeline = collectionView.dequeueReusableSupplementaryView(
                    ...
                    withReuseIdentifier: kind,
                    for: indexPath
                ) as? AWCurrentTimelineSection {
                    view = getSectionTypeCurrentTimeline(
                        timeline: currentTimeline,
                        indexPath: indexPath
                    )
                }
...
extension AWWeekView: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {

    /// Get scrolling direction when first time start beginning dragging until scrolling ends
    internal func getBeginDraggingScrollDirection() -> AWWeekViewScrollDirection {
        ...
        if abs(velocity.x) >= abs(velocity.y) {
            ...
            return AwesomeWeekKit.AWWeekViewScrollDirection(
                direction: .horizontal,
                lockedAt: velocity.x == 0 ? nil : self.collectionView.contentOffset.y
            )
        } else {
            return AwesomeWeekKit.AWWeekViewScrollDirection(
                direction: .vertical,
                lockedAt: velocity.y == 0 ? nil : self.collectionView.contentOffset.x
            )
        }
    }
    ...
    open func scrollViewWillEndDragging(
        ...
        guard let AWWeekViewScrollDirection = self.AWWeekViewScrollDirection,
            AWWeekViewScrollDirection.direction == .horizontal
        else { return }
        paginationEffect(
            scrollView,
    ...
    private func endOfScroll() {
        ...
        loadPage()
        self.AWWeekViewScrollDirection = nil
    }

    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        guard let AWWeekViewScrollDirection = AWWeekViewScrollDirection else { return }
        if let lockedAt = AWWeekViewScrollDirection.lockedAt {
            if AWWeekViewScrollDirection.direction == .horizontal {
        ...
        guard flowLayout.sectionWidth != nil, AWWeekViewScrollDirection.direction == .horizontal
        else { return }
        checkScrollableRange(contentOffsetX: scrollView.contentOffset.x)
        updateAllDayBar(isScrolling: true)
    }

    private func paginationEffect(
        _ scrollView: UIScrollView,
        withVelocity velocity: CGPoint,
        targetContentOffset: UnsafeMutablePointer<CGPoint>
    ) {
        ...
        let pageWidth: CGFloat =
            scrollType == .section ? flowLayout.sectionWidth : contentViewWidth
        ...
        let currentPage =
            scrollType == .section ? currentSection : (currentSection >= numOfDays ? 1 : 0)  // The divider section for 0 and 1 page is at numOfDays
        ...
        targetContentOffset.pointee = CGPoint(
            x: shouldScrollToContentOffsetX,
    ...
    public func loadPage() {
        ...
        scrollType == .page ? loadPagePageScroll() : loadPageSectionScroll()
    }
...
extension AWWeekView {

    /// Get the section Type current timeline
    public func getSectionTypeCurrentTimeline(
        timeline: AWCurrentTimelineSection,
        indexPath: IndexPath
    ) -> UICollectionReusableView {
        let date = flowLayout.dateForColumnHeader(at: indexPath)
    ...
    public func getPageTypeCurrentTimeline(timeline: AWCurrentTimelinePage, indexPath: IndexPath)
        -> UICollectionReusableView
    {
        let date = flowLayout.dateForColumnHeader(at: indexPath)
    ...
    private func checkScrollableRange(contentOffsetX: CGFloat) {
        if let leftX = scrollableEdges.leftX, contentOffsetX < leftX {
            collectionView.setContentOffset(
                CGPoint(x: leftX, y: collectionView.contentOffset.y),
                animated: false
            )
        }
        ...
        if let rightX = scrollableEdges.rightX, contentOffsetX > rightX {
            collectionView.setContentOffset(
                CGPoint(x: rightX, y: collectionView.contentOffset.y),
                animated: false
            )
        }
    }
        ...
        if let endDate = scrollableRange.endDate, endDate < currentPageFirstDate {
            ...
            if let startDate = scrollableRange.startDate {
                if startDate >= currentPageFirstDate {
                    ...
                    if scrollType == .page || startDate <= firstDateInView {
        ...
        if let startDate = scrollableRange.startDate, startDate > currentPageLastDate {
            ...
            if let endDate = scrollableRange.endDate {
                if endDate <= currentPageLastDate {
                    ...
                    if scrollType == .page || endDate >= lastDateInView {
                        ...
                        scrollableEdges.rightX =
                            (flowLayout.sectionWidth ?? 0) * CGFloat(days - numOfDays + 1)
                    }
                }
            } else {
                scrollableEdges.rightX = nil
...
extension AWWeekView: WeekViewFlowLayoutDelegate {
    ...
    public func collectionView(
        ...
        if let events = allEventsBySection[date] {
            let event =
                isAllDaySupported
                ? notAllDayEventsBySection[date]![indexPath.item] : events[indexPath.item]
    ...
    public func collectionView(
        ...
        return AWSupplementaryViewKind.eventCell
    }
}
